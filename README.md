...
`install-ubuntu.sh` 必须以 root 运行。脚本会在同一次执行中创建普通用户（默认 `dev`），并把 CLI 工具、`claude` / `codex`、zsh/bash/Vim 配置安装到这个用户下，同时把该用户默认 shell 设为 zsh。
...
| Shell 配置 | PowerShell profile、PSFzf 快捷键、git 查看类函数 | `~/.zprofile`、`~/.zshrc`、`~/.bashrc.d/cli-setup.bash`、fzf 快捷键、git 查看类 alias |
...
5. 以该普通用户身份安装用户级工具和配置，包括 `claude`、`codex`、npm 全局包、zsh/bash 配置和 Vim 配置。
6. 将该普通用户的默认登录 shell 设置为 `/usr/bin/zsh`。
