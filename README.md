...
```powershell
.\install.ps1 -NoProfile   # 只安装工具，不写入 PowerShell profile / _vimrc
.\install.ps1 -NoSsh       # 跳过 OpenSSH Server 配置
```
...
`install-ubuntu.sh` 必须以 root 运行。脚本会在同一次执行中创建普通用户（默认 `dev`），并把 CLI 工具、`claude` / `codex`、bash/Vim 配置安装到这个用户下。
...
```bash
bash ./install-ubuntu.sh --no-profile          # 只安装工具，不写入 bash/vim 配置
bash ./install-ubuntu.sh --no-ssh              # 跳过 OpenSSH Server 配置
CLI_USER=alice bash ./install-ubuntu.sh        # 指定自动创建的普通用户，默认 dev
CLI_PASSWORD=secret bash ./install-ubuntu.sh   # 指定控制台登录密码
NODE_MAJOR=22 bash ./install-ubuntu.sh         # 指定 Node.js 主版本，默认 24
```

如果未指定 `CLI_PASSWORD`，脚本会生成随机强密码，并在执行末尾打印一次。请当场记录，之后建议用 `passwd` 修改。
...
|------|---------|---------------|
| 包管理器 | Scoop | apt |
| Shell | PowerShell 7 (`pwsh`) | bash |
| 基础工具 | `git`, `gh`, `jq`, `7z` | `git`, `gh`, `jq`, `7z`, `build-essential`, `unzip` |
...
| Shell 配置 | PowerShell profile、PSFzf 快捷键、git 查看类函数 | `~/.bashrc.d/cli-setup.bash`、fzf 快捷键、git 查看类 alias |
| Vim 配置 | `~/_vimrc`、持久 undo 目录 | `~/.vimrc`、持久 undo 目录 |
| SSH | 可配置 OpenSSH Server | 默认配置 OpenSSH Server，除非 `--no-ssh` |

脚本均设计为幂等执行：已存在的工具和配置会尽量复用或跳过；覆盖用户配置前会备份到带时间戳的 `.bak-*` 文件。

`add-admin-ssh-key.ps1` 也是幂等的：同一把公钥已存在时不会重复追加，并会修正 `administrators_authorized_keys` 的 ACL。
...
3. 设置控制台登录密码。只在账户当前没有密码时设置，重复运行不会覆盖你改过的密码。
4. 创建 `~/.ssh` 和空的 `~/.ssh/authorized_keys`，权限分别为 `0700` 和 `0600`。
5. 以该普通用户身份安装用户级工具和配置，包括 `claude`、`codex`、npm 全局包、bash 配置和 Vim 配置。
...
把公钥写入目标用户的 `~/.ssh/authorized_keys` 后即可连接：
...
| 快捷键 | Windows PowerShell 7 | Ubuntu bash |
|--------|----------------------|-------------|
...
├── add-admin-ssh-key.ps1                    # Windows 管理员 SSH 公钥追加脚本
├── install-ubuntu.sh                        # Ubuntu Server 一键安装脚本
├── config/
│   ├── Microsoft.PowerShell_profile.ps1     # PowerShell 7 配置
...
1. Windows lint：解析 `install.ps1`、`add-admin-ssh-key.ps1` 和 PowerShell profile，并运行 PSScriptAnalyzer。
2. Ubuntu lint：对 `install-ubuntu.sh` 和配置运行 `bash -n` / ShellCheck。
3. Windows install：在 `windows-latest` 上执行 `.\install.ps1 -NoSsh` 并验证工具、Node.js、AI CLI、profile 和 git 快捷方式。
4. Ubuntu install：在 `ubuntu-latest` 上以 root 执行 `install-ubuntu.sh --no-ssh`，验证自动建用户、sudo、密码、SSH 文件权限、CLI、Node.js、AI CLI、bash 配置、fzf 绑定和 git 快捷方式。
...
- Ubuntu 会生成 `en_US.UTF-8`、`zh_CN.UTF-8`，以及本次 SSH 会话转发进来的 UTF-8 locale，用于消除 `setlocale: cannot change locale` 警告。
