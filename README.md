...
| PowerShell 7 | `pwsh` | 安装 | 不安装 | Windows SSH 默认 shell 使用 |
...
1. 安装 Scoop（若未安装）
2. 确保 PowerShell 7 (`pwsh`) 可用
3. 安装 git（Scoop 添加 bucket 需要）
4. 添加 `extras` bucket
5. 安装常用 CLI 工具
6. 确保 Node.js LTS、`npm`、`npx` 可用
7. 安装 Codex CLI 与 Claude Code CLI：优先使用官网安装脚本，失败后使用 npm 包兜底
8. 安装 `PSFzf` 模块
9. 复制 `config/Microsoft.PowerShell_profile.ps1` 到 PowerShell 7 的 profile 路径
10. 复制 `config/_vimrc` 到 `$HOME\_vimrc`，并创建 `vimfiles\undo`
11. 管理员会话下配置 OpenSSH Server：启用 `sshd`、仅监听 `58888`、禁用密码登录（仅密钥）、限制登录组、放行防火墙、把 SSH 默认 shell 设为 `pwsh`
...
Windows 侧 PowerShell 7 (`pwsh`) 与 Ubuntu bash 都提供同名快捷方式。
...
| 快捷键 | Windows `pwsh` | Ubuntu bash |
|--------|--------------------|-------------|
| Ctrl+R | 模糊搜索命令历史 | 模糊搜索命令历史 |
| Ctrl+T | 模糊查找当前目录文件 | 模糊查找当前目录文件 |
| Ctrl+D | 空命令行退出 shell | bash 默认 EOF 行为 |

Ubuntu 侧会在交互式 bash 中 source fzf 的 `key-bindings.bash` 与 `completion.bash`，为 Ctrl+R / Ctrl+T 提供和 Windows 侧 PowerShell 7 相同的模糊历史/文件选择体验。
